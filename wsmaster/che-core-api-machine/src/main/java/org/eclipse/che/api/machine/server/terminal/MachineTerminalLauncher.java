...
public class MachineTerminalLauncher {
    ...
    private final Map<String, MachineSpecificTerminalLauncher> terminalLaunchers;

    @Inject
    public MachineTerminalLauncher(EventService eventService,
                                   MachineManager machineManager,
                                   Set<MachineSpecificTerminalLauncher> machineSpecificTerminalLaunchers) {
        this.eventService = eventService;
        this.machineManager = machineManager;
        this.terminalLaunchers = machineSpecificTerminalLaunchers.stream()
                                                                 .collect(Collectors.toMap(MachineSpecificTerminalLauncher::getMachineType,
                                                                                           Function.identity()));
    }
            ...
            public void onEvent(MachineStatusEvent event) {
                if (event.getEventType() == MachineStatusEvent.EventType.RUNNING) {
                    try {
                        ...
                        MachineSpecificTerminalLauncher machineSpecificTerminalLauncher = terminalLaunchers.get(machine.getConfig().getType());
                        if (machineSpecificTerminalLauncher == null) {
                            LOG.warn("Terminal launcher implementation was not found for machine {} with type {}.",
                                     machine.getId(),
                                     machine.getConfig().getType());
                        } else {
                            machineSpecificTerminalLauncher.launchTerminal(machine);
                        }
